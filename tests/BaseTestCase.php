...
abstract class BaseTestCase extends TestCase
{
    ...
    /**
     * Asserts that $image is a fully loaded Image with a valid driver and resource.
     */
    public function assertImageLoaded(Image $image): void
    {
        $this->assertInstanceOf(Image::class, $image);
        $this->assertNotNull($image->driver, 'Image driver should not be null after loading.');
        $this->assertInstanceOf(
            'meriksk\PhpImage\driver\Image' . ucfirst(static::$lib),
            $image->driver
        );
        $this->assertResource($image->getResource());
    }

    /**
     * Asserts that $resource is a valid image resource for the current driver.
     * @param mixed $resource
     * @param string $message
     */
    public function assertResource($resource, string $message = ''): void
    {
        $msg = !empty($message) ? $message : 'Invalid resource.';

        if (static::$lib === Image::DRIVER_IMAGICK) {
            $this->assertInstanceOf('Imagick', $resource, $msg);
            return;
        }

        if (\PHP_VERSION_ID >= 80000) {
            $this->assertInstanceOf(\GdImage::class, $resource, $msg);
        } else {
            $this->assertTrue(
